...
public abstract class FeatureController {
     ...
     * 上了30的是否需要判断此权限
     */
    private boolean ignoreCheckPermissionsMANAGE_EXTERNAL_STORAGE = false;
     ...
     * 是否忽略权限检查
     */
    public void ignoreCheckPermissionsMANAGE_EXTERNAL_STORAGE() {
        this.ignoreCheckPermissionsMANAGE_EXTERNAL_STORAGE = true;
    }
    ...
    private boolean checkPermission() {
        if (AriaConfig.getInstance().getAPP().checkCallingOrSelfPermission(Manifest.permission.INTERNET) != PackageManager.PERMISSION_GRANTED) {
            ALog.e(TAG, "启动失败，缺少权限：Manifest.permission.INTERNET");
            return false;
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            if (ignoreCheckPermissionsMANAGE_EXTERNAL_STORAGE && !Environment.isExternalStorageManager()) {
                ALog.e(TAG, "启动失败，缺少权限：Manifest.permission.MANAGE_EXTERNAL_STORAGE");
                return false;
            }
        } else {
            if (AriaConfig.getInstance().getAPP().checkCallingOrSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                ALog.e(TAG, "启动失败，缺少权限：Manifest.permission.WRITE_EXTERNAL_STORAGE");
                return false;
            }
            if (AriaConfig.getInstance().getAPP().checkCallingOrSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
